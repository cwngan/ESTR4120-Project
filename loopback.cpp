...
void input_data_callback(ma_device *pDevice, void *pOutput, const void *pInput,
  ...
  void *write_ptr;
  size_t bytesToWrite = (MAX_BITRATE / 8) / (SAMPLE_RATE / frameCount);
...
void output_data_callback(ma_device *pDevice, void *pOutput, const void *pInput,
  ...
  ma_rb *ringBuffer = cb_data->ring_buffer;
  void *read_ptr;
  size_t bytesToRead = (MAX_BITRATE / 8) / (SAMPLE_RATE / frameCount);
...
int loopback_audio(OpusEncoder *encdoer_state, OpusDecoder *decoder_state) {
  ...
  if (ma_rb_init(MAX_BITRATE / 8, NULL, NULL, ringBuffer) != MA_SUCCESS)
    return -1;
  ...
  input_device_config.capture.pDeviceID = &pCaptureInfos[capture_device].id;
  ...
  input_device_config.periodSizeInFrames = 960;
  ...
  output_device_config.playback.pDeviceID = &pPlaybackInfos[playback_device].id;
  ...
  output_device_config.periodSizeInFrames = 960;
